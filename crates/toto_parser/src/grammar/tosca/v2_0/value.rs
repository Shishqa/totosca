...
impl Parse for Value {
    fn parse(ctx: &mut Context, n: &yaml_peg::NodeRc) -> GraphHandle {
        match n.rc_ref().as_ref() {
            ...
            yaml_peg::Yaml::Int(_) => Integer::parse(ctx, n),
            yaml_peg::Yaml::Float(f) => ctx.graph.add_node(Entity::Float(f.to_string())),
            yaml_peg::Yaml::Str(_) => String::parse(ctx, n),
            yaml_peg::Yaml::Bool(_) => Boolean::parse(ctx, n),
            yaml_peg::Yaml::Seq(_) => List::<Value>::parse(ctx, n),
            yaml_peg::Yaml::Map(_) => Map::<Value, Value>::parse(ctx, n),
            // TODO: handle anchors
            _ => unimplemented!(),
        }
    }

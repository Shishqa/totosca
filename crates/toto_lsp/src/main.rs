                ...
                lsp_server::Message::Request(req) => {
                    ...
                    match req.method.as_str() {
                        lsp_types::request::GotoDefinition::METHOD => {
                            let params: lsp_types::GotoDefinitionParams = from_value(req.params)?;
                            self.goto_definition(&params.text_document_position_params)?;
                            continue;
                        }
                        &_ => {}
                    }
        ...
        let diagnostics: Vec<lsp_types::Diagnostic> = get_errors(&self.ast)
            .filter_map(|(what, loc)| {
                ...
                let get_lc = |offset| -> (u32, u32) {
                    ...
                    let charno =
                        pos - linebreaks.iter().next_back().copied().unwrap_or_default() - 1;
